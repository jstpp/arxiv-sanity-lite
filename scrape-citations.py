...
from db.SQLLite.OrmDB import Citation
...
def scrape(ID):
    ...
    with engine.connect() as session:
        stmt = select(Citation).where(Citation.origin_publication_id == ID).limit(1)
...
def main():
    ...
    for key in keys:
        ...
        for citation in result:
            with engine.connect() as session:
                stmt = insert(Citation).values(origin_publication_id=key, citation_publication_id=citation)
